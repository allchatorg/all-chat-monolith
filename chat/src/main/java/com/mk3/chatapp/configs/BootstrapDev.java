...
public class BootstrapDev implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (isFirstStart()) {
            ...
            userCreationService.createUsersAndChatrooms();

            // Optional: Create bulk test messages (uncomment to enable)
//            messageBatchEnteringService.createBulkTestMessages();
        ...
        log.info("Successfully started the application in DEV mode.");
    }

    private boolean isFirstStart() {
        return userRepository.count() == 0 && chatRoomRepository.count() == 0;
    }
        ...
        for (String tagName : tagNames) {
            Tag tag = tagRepository.findByNameIgnoreCase(tagName)
                    .orElseGet(() -> tagRepository.save(
                            Tag.builder()
                                    .name(tagName)
                                    .restrictedToAdults(tagName.equals("explicit")
                                            || tagName.equals("gore/harm"))
                                    .build()));
